      ...
      if (Array.isArray(element)) {
        element.forEach(({ title, content }, i) => {
          ...
          content.forEach((item, j) => {
            ...
            fd.append(
              `video${i}`,
              fs.createReadStream(item.video.path),
              item.video.name
            );
        ...
        if (key === 'thumnail') {
          fd.append(key, fs.createReadStream(element.path), element.name);
    ...
    try {
      const response = await axios.post('/api/course', fd, {
        headers: {
          ...fd.getHeaders(),
          Authorization:
            'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjVlZDEyMjM3MzhmZjUwNDJjOGE3YjlmMiIsImlhdCI6MTU5MDk5NjAxOCwiZXhwIjoxNTk4NzcyMDE4fQ.G7glpbc-yYeYGfM7qZYkxvpPSN0mXkWeeiFkCNY-wdg',
        },
